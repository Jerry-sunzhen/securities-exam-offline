...
const questionPayload = {
  meta: {
    ...
    supplementedExplanationCount: allQuestions.filter((question) => question.explanationSource === "local-materials-ai" || question.explanationSource === "official-web").length,
    webVerifiedExplanationCount: allQuestions.filter((question) => question.explanationSource === "official-web").length,
    missingExplanationCount: allQuestions.filter((question) => !String(question.explanation || "").trim() || /原资料未提供可用解析/.test(String(question.explanation || ""))).length,
